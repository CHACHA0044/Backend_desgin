      ...
      <nav ref={navRef} className={`card-nav ${themeClass} ${isExpanded ? 'open' : ''} ${scrolled ? 'scrolled' : ''}`}>
        <div className="card-nav-top">
          ...
          <div className="logo-container">
            <Link to="/" className="card-nav-logo-text" style={{ color: textColor }} tabIndex={-1} onClick={closeNav}>SBS</Link>
          </div>
            ...
            <div key={`${item.label}-${idx}`} className="nav-card" ref={setCardRef(idx)} style={{ backgroundColor: item.bgColor, color: item.textColor }}>
              ...
              <div className="nav-card-links">
                {item.links?.map((lnk, i) => (
                  <Link key={i} className="nav-card-link" to={lnk.to || '#'} style={{ color: item.textColor }} onClick={closeNav}>
...
const getNavItems = (dark) => [
  { label: 'Navigate', bgColor: dark ? '#0e0e0e' : '#f2f2f2', textColor: dark ? '#fff' : '#000', links: [{ label: 'Home', to: '/' }, { label: 'About', to: '/about' }] },
  { label: 'Account',  bgColor: dark ? '#141414' : '#eaeaea', textColor: dark ? '#fff' : '#000', links: [{ label: 'Login', to: '/login' }, { label: 'Register', to: '/register' }] },
  { label: 'Dashboard',bgColor: dark ? '#1a1a1a' : '#e2e2e2', textColor: dark ? '#fff' : '#000', links: [{ label: 'Tasks', to: '/dashboard' }, { label: 'Profile', to: '/profile' }] },
];
...
export default function Profile() {
  ...
  return (
    <div className={`relative min-h-screen overflow-x-hidden transition-colors duration-500 ${bg} ${text}`}>
      ...
      <CustomCursor />
      <CardNav items={getNavItems(dark)} dark={dark} ease="power3.out" onOpenChange={setNavOpen} onToggleTheme={toggleTheme} />
          ...
          <div className="relative z-10">
            ...
            >
              ...
              <div className="ml-auto shrink-0">
                <span
                  className={`rounded-xl border px-3 py-1.5 text-[10px] font-bold tracking-widest uppercase ${
                    role === 'admin'
